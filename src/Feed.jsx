...
// Since the launcher assigns dynamic ports, the frontend needs to know where its backend is
...
export default function Feed({ userId, onLogout, onDestroy }) {
  ...
  const [peerCount, setPeerCount] = useState(0); // US15
  const [sortBy, setSortBy] = useState('newest'); // US6
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === 'NEW_NODE') fetchPosts();
  ...
  const handleVote = async (parentId, voteValue) => {
    try {
      ...
      if (res.status === 403 || res.status === 429) { const data = await res.json(); triggerError(data.error); }
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 text-white relative">
      {errorMsg && <div className="fixed top-4 left-1/2 -translate-x-1/2 w-max max-w-sm text-center bg-red-600 text-white px-6 py-3 rounded-xl font-bold z-40 animate-bounce">{errorMsg}</div>}
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onLogout} className="text-sm font-bold text-gray-400 hover:text-white">Log Out</button>
          <button onClick={onDestroy} className="text-sm font-bold text-red-400 hover:text-red-300 border border-red-900/50 px-3 py-1 rounded-lg bg-red-900/20">Destroy</button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {displayedPosts.length === 0 && <p className="text-center text-gray-500 py-8">No content matches your filters.</p>}
      </div>
    </div>
  );
}